...
function MovilesRegistro() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:3001/moviles_agregar', formData);
  ...
  const filterMoviles = () => {
    return moviles.filter(movil => 
      ...
      (filtros.estado_id ? movil.estado_id.toString() === filtros.estado_id : true) &&
      (filtros.vencido === 'Sí' ? isServiceVencido(movil.fecha_service) : filtros.vencido === 'No' ? !isServiceVencido(movil.fecha_service) : true)
    );
  };
  ...
  const paginatedMoviles = filteredMoviles.slice((paginaActual - 1) * itemsPorPagina, paginaActual * itemsPorPagina);
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:3001/moviles_editar/${editData.id}`, cambios);
    ...
    <div className="moviles-registro-container">
      ...
      <div className="filtros">
        ...
        <input
          ...
          onChange={handleChangeFiltros}
        />
        <select name="estado_id" value={filtros.estado_id} onChange={handleChangeFiltros}>
          <option value="">Filtrar por Estado</option>
          {estados.map(estado => (
            <option key={estado.id} value={estado.id}>{estado.nombre}</option>
          ))}
        </select>
        <select name="vencido" value={filtros.vencido} onChange={handleChangeFiltros}>
      ...
      <table className="moviles-registro-tabla">
        ...
        <tbody>
          {paginatedMoviles.map((movil) => (
            <tr key={movil.id} style={isServiceVencido(movil.fecha_service) ? { backgroundColor: 'red' } : {}}>
              <td>{movil.interno}</td>
              <td>{movil.marca}</td>
              <td>{movil.modelo}</td>
              <td>{movil.dominio}</td>
              ...
              <td>
                <button
                  ...
                  className={movil.estado_id === 3 ? 'editar-deshabilitado' : 'agregar-movil-button'}
                  disabled={movil.estado_id === 3}
                  title={movil.estado_id === 3 ? 'Este móvil está dado de baja y no se puede editar.' : ''}
                >
                  Editar
                </button>
              </td>
      ...
      <div className="paginacion">
        ...
        <button onClick={() => setPaginaActual(prev => Math.min(prev + 1, totalPages))} disabled={paginaActual === totalPages}>Siguiente</button>
      </div>
        ...
        <div className="modal-overlay">
          <div className="modal">
            <h2>Agregar Móvil</h2>
              ...
              <label>Interno:</label>
              <input type="text" name="interno" value={formData.interno} onChange={handleChangeFormData} required />
              <label>Marca:</label>
              <input type="text" name="marca" value={formData.marca} onChange={handleChangeFormData} required />
              <label>Modelo:</label>
              <input type="text" name="modelo" value={formData.modelo} onChange={handleChangeFormData} required />
              <label>Dominio:</label>
              <input type="text" name="dominio" value={formData.dominio} onChange={handleChangeFormData} required />
              <label>VIN:</label>
              ...
              <label>Estado:</label>
              <select name="estado_id" value={formData.estado_id} onChange={handleChangeFormData} required>
