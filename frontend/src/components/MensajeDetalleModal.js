...
function MensajeDetalleModal({ mensaje, onClose, tipo }) {
  useEffect(() => {
    if (mensaje && mensaje.id && tipo === 'recibido') {
      axios.put(`http://localhost:3001/mensajes/marcar-leido/${mensaje.id}`)
  ...
  return (
    <div className="modal-overlay">
      <div className="modal-content">
        ...
        <p><strong>{tipo === 'recibido' ? 'Remitente' : 'Destinatario'}:</strong> {origen}</p>
        <p><strong>Fecha:</strong> {new Date(mensaje.fecha_envio).toLocaleString()}</p>
        <hr />
