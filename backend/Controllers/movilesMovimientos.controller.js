...
const registrarSalida = (req, res) => {
  ...
  db.query('SELECT kilometraje_actual FROM moviles WHERE id = ?', [movil_id], (err, rows) => {
    if (err || rows.length === 0) {
      return res.status(500).json({ error: 'Error al obtener el kilometraje actual del móvil' });
    }
    ...
    const query = `
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `;
    db.query(query, [movil_id, fecha_salida, chofer_id, destino, km_salida, jefe_dotacion], (err, result) => {
      if (err) {
      ...
      if (Array.isArray(dotacion) && dotacion.length > 0) {
        const dotacionQuery = `
          ...
          VALUES ?
        `;
        const dotacionValues = dotacion.map(personal_id => [movimiento_id, personal_id]);
...
const registrarRetorno = (req, res) => {
  ...
  db.query('SELECT km_salida FROM moviles_movimientos WHERE id = ?', [id], (err, rows) => {
    if (err || rows.length === 0) {
      return res.status(500).json({ error: 'Error al validar kilometraje de salida' });
    }
    const km_salida = rows[0].km_salida;
    if (kilometraje_final < km_salida) {
      return res.status(400).json({ error: 'El kilometraje final no puede ser menor al de salida' });
    }
    ...
    db.query(updateMovimiento, [fecha_retorno, kilometraje_final, novedades, id], (err) => {
      if (err) {
        console.error('Error al registrar retorno:', err);
        return res.status(500).json({ error: 'Error al registrar retorno' });
      }

    db.query('SELECT movil_id FROM moviles_movimientos WHERE id = ?', [id], (err2, rows2) => {
      if (err2 || rows2.length === 0) {
        console.error('Error al obtener el ID del móvil:', err2);
        return res.status(500).json({ error: 'No se pudo determinar el móvil' });
      }
      ...
      db.query(
        ...
        (err3) => {
          if (err3) {
            ...
            return res.status(500).json({ error: 'Error al actualizar kilometraje' });
          }
...
const obtenerMovilesEnSalida = (req, res) => {
  ...
  db.query(query, (err, results) => {
    if (err) {
      ...
      return res.status(500).json({ error: 'Error al obtener móviles en salida' });
    }
